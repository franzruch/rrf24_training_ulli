...
*------------------------------------------------------------------------------- 
	...
	* Also export in tex for latex
	esttab 	all dist_1 dist_2 dist_3 ///
			...
			label ///
			main(mean %6.2f) aux(sd) ///
			refcat(hh_size "\textbf{HH characteristics}" drought_flood "\textbf{Shocks:}" , nolabel) /// /*These are subheading*/
			mtitle("Full Sample" " Kibaha" "Bagamoyos" "Chamwino") ///
			nonotes addn(Mean with standard deviations in parentheses.)
...
*------------------------------------------------------------------------------- 				
	...
	* Export results
	esttab 	model1 model2 model3 ///
			using "$outputs/regressions.tex" , /// /*You can change this to csv*\
			label ///
			b(%9.3f) se(%9.3f) ///
			nomtitles ///
			mgroup("Annual food consumption(USD)", pattern(1 0 0 ) span) /// /*Span says put title across 3 rows */
